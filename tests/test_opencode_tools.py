...
class TestDispatchModeGate(OpencodeTestBase):
    ...
    def test_denied_in_read_mode(self, mock_request):
        self._set_mode("READ")
        result = opencode_tools.opencode_dispatch("fix the bug", confirm=True)
    ...
    def test_denied_in_run_mode_without_confirm(self, mock_request):
        self._set_mode("RUN")
        result = opencode_tools.opencode_dispatch("fix the bug", confirm=False)
        self.assertEqual(result["status"], "denied")
        mock_request.assert_not_called()
    ...
    def test_empty_prompt_rejected_before_mode_check(self):
        ...
        result = opencode_tools.opencode_dispatch("   ", confirm=True)
    ...
    def test_allowed_in_run_mode_with_confirm_dispatches_session_then_prompt(self, mock_request):
        ...
        result = opencode_tools.opencode_dispatch("fix the stuck-message backlog", confirm=True)
